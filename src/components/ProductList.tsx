...
const Card = (props: CardProps) => {
  const { product } = props;
  const { productName, price, imgSrc } = product;
  ...
  return (
    ...
    >
      {products.map((product) => {
        return <Card key={product.productName} product={product} />;
      })}
    </div>
  );
};
